...
struct AdminView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            if self.Admin == true{
                Text("Welcome to the Admin Dashboard").onAppear(){self.readStatistics()}
                if self.AdminView == true {
                    Text("Statistics Below: ")
                    Text("Total Green: \(self.totalGreen)")
                    Text("Total Yellow: \(self.totalYellow)")
                    Text("Total Red: \(self.totalRed)")
                    Text("Total Late Surveys: \(self.totalLate)")
                    Text("Total Surveys Completed Today: \(self.totalOnTime)")
                }
            }
        else{
